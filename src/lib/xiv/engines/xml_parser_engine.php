...
class xml_parser_engine {
	...
	public function node_filter(&$node, $filter) {

		$nodes = array();

		foreach($node['_nodes'] as $i => $v) {
			$test =& $node['_nodes'][$i][$filter];
			if (isset($test)) {
				$nodes[] = array(array('_nodes' => $test));
			}
		}
	...
	public function get_value($path) {
		...
		$nodes = $this->get_nodes($path);
		if ($nodes) {
			foreach($nodes['_nodes'] as $i => $v) {
				$parent_node =& $nodes['_nodes'][$i]['module'];
				if (isset($parent_node)) {
					foreach($parent_node['_nodes'] as $j => $w) {
						$node =& $parent_node['_nodes'][$j][$final];
						if (isset($node)) {
							return $node['_data'];
						}
					}
