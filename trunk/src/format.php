...
class format extends html_filter_engine {
	...
	public function smiley($text) {
		...
		if ($conf->get('core/enable_smileys', true, 'b')) {

			$smiley_theme = $conf->get('core/smiley_theme', 'default', 's');

			$theme = TM_MEDIA_DIR.'smileys/'.$smiley_theme.'/theme.ini';
			...
			if (file_exists($theme)) {
				...
				if (!empty($data['icons'])) {
					foreach($data['icons'] as $icon => $pattern) {
						...
						$callback = create_function('$a', 'return isset($a[3]) ? "<img src=\"'.TM_BASE_URL.'media/smileys/'.$smiley_theme.'/'.$icon.'.'.$data['theme']['extension'].'\" width=\"'.$data['theme']['width'].'\" height=\"'.$data['theme']['height'].'\" />" : $a[0];');
	...
	public function display($text, $user_id = 0) {
		...
		return $this->smiley($this->display_with_filter($text, $filter_level));
	}
