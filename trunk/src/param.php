...
/**
 * Param
 * Text_motion request variables wrapper.
 * ---
 * Written by Jose Carlos Nieto <xiam@astrata.com.mx>
 * Copyright (c) 2007 Text_motion Development Crew
 *
 * Licensed under The MIT License
 * Redistributions of files must retain the above copyright notice.
 *
 * @author          Jose Carlos Nieto <xiam@astrata.com.mx>
 * @copyright       Copyright (c) 2007, Text_motion Development Crew
...
class param extends url_handler {
	...
	*/
	...
	public function upload($varname, $destdir) {
		...
		$varpath = explode('.', $varname);

		$file = $files;
		while ($varpath) {
			$file =& $file[array_shift($varpath)];
		}
		...
		if (!empty($file)) {
			...
			foreach ($file as $i => $f) {
				$file[$i] = $this->move_uploaded_file($f, $destdir);
				$file[$i] = $file[$i][0];
			}
			return $file;
		} 
